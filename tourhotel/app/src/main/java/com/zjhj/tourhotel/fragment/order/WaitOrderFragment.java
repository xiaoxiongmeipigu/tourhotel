...
public class WaitOrderFragment extends BaseFrag {
    ...
    private void initView() {
        ...
        mAdapter = new OrderWaitAdapter(getActivity(), mList);
        ...
        passDialog.setLeftBtnText("取消").setRightBtnText("确认").setTitle("确认通过该订单吗？");
    ...
    private void initListener() {
        ...
        cancelAlertDialog.setRightClickListener(new View.OnClickListener() {//提交
            ...
            public void onClick(View view) {
                ...
                if(pos>=0)
                    orderreject(cancelAlertDialog.getInfo());
                cancelAlertDialog.dismiss();
            }
        });
    ...
    public void load() {
        showLoading();
        ItemApi.orderindex(getActivity(),"1", pageIndex + "", pageNum + "",new RequestPageCallback<List<MapiOrderResult>>() {
            @Override
    ...
    private void orderreject(String content){
        MapiOrderResult orderResult =  mList.get(pos);
        showLoading();
        ItemApi.orderreject(getActivity(), orderResult.getId(), content, new RequestCallback() {
            @Override
    ...
    private void pass(){
        MapiOrderResult orderResult =  mList.get(pos);
        showLoading();
        ItemApi.orderadopt(getActivity(), orderResult.getId(),"1", new RequestCallback() {
            @Override
            public void success(Object success) {
                hideLoading();
                mList.remove(pos);
